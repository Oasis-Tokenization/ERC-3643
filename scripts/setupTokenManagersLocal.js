...
async function main () {
    ...
    let tir = await TIR.attach(process.env.LOCAL_TIR);
    let ctr = await CTR.attach(process.env.LOCAL_CTR);
    let mc = await MC.attach(process.env.LOCAL_MC);
    let irs = await IRS.attach(process.env.LOCAL_IRS);
    let ir = await IR.attach(process.env.LOCAL_IR);
    let token = await TOKEN.attach(process.env.LOCAL_TOKEN);
    let gateway = await GATEWAY.attach(process.env.LOCAL_GATEWAY);

    let agentManager = await AGENTMNGR.deploy(process.env.LOCAL_TOKEN);
    console.log("AgentManager addr: ", agentManager.target, " Linked to Token : ", process.env.LOCAL_TOKEN);
    await sleep(3000);
    ...
    await agentManager.addSupplyModifier(otaControllerIdentityAddr);
    console.log("AgentManager: Added Supply Modifier : ", otaControllerIdentityAddr);
    await sleep(3000);
    await agentManager.addWhiteListManager(otaControllerIdentityAddr);
    console.log("AgentManager: Added WhiteList Manager: ", otaControllerIdentityAddr);
    await sleep(3000);
    await agentManager.addFreezer(otaControllerIdentityAddr);
    console.log("AgentManager: Added Freezer: ", otaControllerIdentityAddr);
    await sleep(3000);
    await agentManager.addTransferManager(otaControllerIdentityAddr);
    console.log("AgentManager: Added TransferManager: ", otaControllerIdentityAddr);
    await sleep(3000);
    await agentManager.addRecoveryAgent(otaControllerIdentityAddr);
    console.log("AgentManager: Added RecoveryAgent: ", otaControllerIdentityAddr);
    await sleep(3000);
    await agentManager.addComplianceAgent(otaControllerIdentityAddr);
    console.log("AgentManager: Added Compliance Agent: ", otaControllerIdentityAddr);

    let ownerManager = await OWNERMANAGER.deploy(process.env.LOCAL_TOKEN);
    console.log("OwnerManager addr : ", ownerManager.target, " Linked to Token : ", process.env.LOCAL_TOKEN);
    await sleep(3000);
